 ...
 * primes-excep.cl
 *
 * Primes modified with Exceptions, kgao@eecs
 *
   ...
   try
	...
	try 
	  ...
	  if testee < divisor * divisor 
            then throw "Stop"	-- can stop if divisor > sqrt(testee).
	  else if testee - divisor*(testee/divisor) = 0 
            then throw "Stop"   -- can stop if divisor divides testee.
          else 
          divisor <- divisor + 1
	  fi fi
         pool	
	   ...
	   out_int(out); out_string(" is prime.\n");	
	};
	
        if stop <= testee then 
          throw "Abort"		-- throw an exception to stop.
        else 
          "continue"
        fi;       
    ...
    pool
   	...
   	out_string("Primes Search Terminated\n");
